...
void main() {
  ...
  group('Airplane Mode - Push (Local Wins)', () {
    test(
      'local trip gets pushed to remote when server has no copy',
      () async {
        ...
        await database.tripsDao.updateTrip(TripsCompanion(
  ...
  group('Airplane Mode - Pull (Remote Wins)', () {
    test(
      'remote overwrites local when remote updatedAt is newer',
      () async {
        ...
        // Verifica stato pre-sync
        final preSyncTrip =
            await database.tripsDao.getTripById('trip-conflict-1');
        ...
        verifyNever(() => mockRemote.upsertTrip(
              any(),
              sentryTrace: any(named: 'sentryTrace'),
            ));
