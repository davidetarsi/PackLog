...
Future<PackageInfo> _stubFactory({
  String version = '1.2.3',
  String buildNumber = '42',
}) async => _stubPackageInfo(version: version, buildNumber: buildNumber);
...
FeedbackUrlService _makeService({
  ...
  packageInfoFactory: () =>
      _stubFactory(version: version, buildNumber: buildNumber),
);
  ...
  group('FeedbackUrlService.build()', () {
    ...
    test('uri contains the app version value as a query parameter', () async {
      ...
      expect(
        ...
        contains('2.0.0+7'),
        reason:
            'La versione "version+buildNumber" deve essere un query parameter',
      );
    });
    ...
    test(
      'build() is idempotent: same OS and parameter keys across calls',
      () async {
    ...
    test(
      'different version stubs produce different appVersion in result',
      () async {
        final r1 = await _makeService(
          version: '1.0.0',
          buildNumber: '1',
        ).build();
        final r2 = await _makeService(
          version: '2.0.0',
          buildNumber: '99',
        ).build();
