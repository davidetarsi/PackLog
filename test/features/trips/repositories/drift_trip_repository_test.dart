...
void main() {
  ...
  group('DriftTripRepository - Complex Aggregation & Mapping Tests', () {
    test(
      'should correctly map TripModel with items and luggages (addTrip + getTripById)',
      () async {
        // === ARRANGE ===
        ...
        // Verify dates
        expect(
          fetchedTrip.departureDateTime
              ?.difference(departureDate)
              .inSeconds
              .abs(),
          lessThanOrEqualTo(1),
        );
        ...
        final sunscreen = fetchedTrip.items.firstWhere(
        ...
        final swimsuit = fetchedTrip.items.firstWhere(
          (item) => item.name == 'Swimsuit',
        );
        ...
        final camera = fetchedTrip.items.firstWhere(
        ...
        expect(
          fetchedTrip.luggages.first.sizeType,
        ...
        expect(
          fetchedTrip.destinationLocation!.displayName,
          equals('Miami Beach, Florida, USA'),
        );
        expect(
          fetchedTrip.destinationLocation!.locationType,
          equals(LocationType.city),
        );
    ...
    test(
      'should correctly map TripModel with multiple luggages (M:N junction table)',
      () async {
        // === ARRANGE ===
    ...
    test(
      'should correctly map TripModel with destinationHouseId instead of location',
      () async {
        // === ARRANGE ===
  ...
  group('DriftTripRepository - CRUD Operations via Repository', () {
    ...
    test(
      'should update trip and replace items and luggages atomically',
      () async {
        // === ARRANGE ===
        ...
        expect(
          fetchedTrip.items.any((item) => item.id == 'original-item'),
          isFalse,
        );
        expect(
          fetchedTrip.items.any((item) => item.id == 'new-item-1'),
          isTrue,
        );
        expect(
          fetchedTrip.items.any((item) => item.id == 'new-item-2'),
          isTrue,
        );
    ...
    test('should delete trip and cascade delete items', () async {
      ...
      final tripItemsAfterDelete = await database.tripsDao.getTripItemsByTripId(
        tripToDelete.id,
      );
