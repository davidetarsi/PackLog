...
void main() {
  ...
  group('BackupController - Filename Validation', () {
    test('should validate correct backup filename', () {
      ...
      expect(
        controller.validateImportFileName(
          '/path/to/stuff-tracker-db-01012025.db',
        ),
        isTrue,
      );
    ...
    test('should reject invalid backup filename', () {
      ...
      expect(controller.validateImportFileName('backup-17022026.db'), isFalse);
  ...
  group('BackupController - Date Formatting', () {
    test('should format date as ddmmyyyy in filename', () async {
      ...
      final dateMatch = RegExp(
        r'stuff-tracker-db-(\d{2})(\d{2})(\d{4})\.db',
      ).firstMatch(fileName);
  ...
  group('BackupController - Import Validation', () {
    test(
      ...
      () async {
        ...
        when(
          () => mockDatabaseBackupService.importData(safetyBackupPath),
        ).thenAnswer((_) async => {});
  ...
  group('BackupController - Disaster Recovery', () {
    test(
      ...
      () async {
        ...
        when(
          () => mockDatabaseBackupService.validateDatabaseFile(validFilePath),
        ).thenAnswer((_) async => true);

        // Mock: Import operation FAILS mid-operation (simulating crash/corruption)
        var importCallCount = 0;
        when(() => mockDatabaseBackupService.importData(any())).thenAnswer((
          invocation,
        ) async {
          importCallCount++;
          final path = invocation.positionalArguments[0] as String;
        ...
        verify(
          () => mockDatabaseBackupService.importData(validFilePath),
        ).called(1);
    ...
    test(
      ...
      () async {
        ...
        ).thenThrow(Exception('Critical filesystem error'));
