...
///
/// Indice su [TripItemEntries.tripId] — la PK è `{id, tripId}` con `id`
/// come colonna principale, quindi le query `WHERE trip_id = ?` necessitano
/// di un indice esplicito sulla seconda colonna.
@TableIndex(name: 'idx_trip_item_entries_trip_id', columns: {#tripId})
...
class TripItemEntries extends Table {
  ...
  /// Categoria dell'oggetto (snapshot). Drift converte automaticamente
  /// la stringa in [ItemCategory] tramite [ItemCategoryConverter].
