...
class _AiClothingSandboxScreenState
  ...
  Future<void> _saveItems() async {
    if (_selectedHouseId == null) {
      _showErrorSnackBar('Seleziona una casa prima di salvare.');
      return;
    }
    ...
    try {
      ...
      final items = _results.map((item) {
        return ItemModel(
          ...
          quantity: 1,
          description: null,
          // Persist every GPT-extracted field as a JSON blob.
          aiMetadata: item.toJson(),
          createdAt: now,
          updatedAt: now,
        );
      }).toList();
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            // ── Save button ─────────────────────────────────────────────────
            ElevatedButton(
              onPressed: (_isLoading || _selectedHouseId == null)
                  ? null
                  : _saveItems,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              '$missing API key(s) not set.\nAggiungi --dart-define=OPENAI_KEY nel launch.json.',
              style: TextStyle(
                fontSize: 12,
                color: Theme.of(context).colorScheme.onTertiaryContainer,
              ),
