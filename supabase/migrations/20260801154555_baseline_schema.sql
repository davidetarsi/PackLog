-- ═════════════════════════════════════════════════════════════════════════════
-- BASELINE — stato dello schema al 2026-08-01
--
-- Prima migration della history. Rappresenta lo schema COMPLETO già presente
-- su entrambi i progetti (PackLogDEV `aluzxhcflmophawoffad` e PackLogPROD
-- `wpmvovzrzciyleszpsnm`) al momento dell'introduzione delle migrations CLI.
--
-- Su entrambi è stata marcata come già applicata con:
--   supabase migration repair --status applied 20260801154555 --linked
-- quindi NON viene rieseguita. Verrebbe eseguita solo su un progetto nuovo
-- e vuoto (`supabase db push`).
--
-- Contenuto identico a `supabase/database_schema.sql` (che resta il file di
-- bootstrap one-shot leggibile), meno `BEGIN;`/`COMMIT;`: la CLI esegue ogni
-- migration già dentro una propria transazione.
--
-- ⚠️  NON modificare questo file: è già applicato ovunque. Ogni cambiamento
--     futuro va in una NUOVA migration (`supabase migration new <nome>`).
-- ═════════════════════════════════════════════════════════════════════════════
